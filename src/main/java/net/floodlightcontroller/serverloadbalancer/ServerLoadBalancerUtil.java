...
public class ServerLoadBalancerUtil {
    ...
    public static AssignmentTree generateAssignmentTreeOptimal(Config config) {
        // Scale and round weights to add up to maxPrefixLength
        List<Integer> normalizedWeights = normalize(config.getWeights(), 1 << config.getMaxPrefixLength());
        // Translate weights into subnet masks that can be assigned to prefixes
        SortedMap<IPv4Address, List<Integer>> masks = generateMasks(normalizedWeights);
        // Assign prefixes
        // TODO refactor
        AssignmentTree assignment = AssignmentTree.balancedTree(config.getMaxPrefixLength());
        assignment.assignMasks(masks);
        return assignment;
    }
