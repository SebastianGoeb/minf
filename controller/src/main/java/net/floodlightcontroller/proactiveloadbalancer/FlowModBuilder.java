...
class FlowModBuilder {
	...
	public static OFFlowAdd buildFlowAdd(IOFSwitch ofSwitch, IPv4AddressWithMask prefix, IPv4Address vip, IPv4Address dip) {
		OFFactory factory = ofSwitch.getOFFactory();
		
		Match match = buildIncomingMatch(factory, vip, prefix);
		List<OFAction> actions = buildIncomingActionList(factory, dip);
		List<OFInstruction> instructions = buildIncomingInstructionList(factory, actions, ofSwitch.getId());
	...
	public static OFFlowDeleteStrict buildFlowDeleteStrict(IOFSwitch ofSwitch, IPv4AddressWithMask prefix, IPv4Address vip, IPv4Address dip) {
		OFFactory factory = ofSwitch.getOFFactory();
	...
	private static List<OFInstruction> buildIncomingInstructionList(OFFactory factory, List<OFAction> actionList,
			DatapathId dpid) {
		switch(factory.getVersion()) {
		case OF_13:
